...
func (s *KetamaSelector) Get(ctx context.Context) (addr grpc.Address, err error) {
	...
	log.Debug(" enter KetamaSelector get, add is ", s.addrs, " ketama key ", ctx.Value)
	...
	if ok {
		...
		log.Infof("------ get target address [%s] for key [%s]", targetAddr, key)
		...
		log.Info("ketama get addr error, ketama key:", s.ketamaKey)
